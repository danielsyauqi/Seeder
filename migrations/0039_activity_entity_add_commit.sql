-- Add 'commit' to the project_activity.entity_type CHECK constraint.
--
-- VCS Sync (Phase 1) writes one collapsed activity row per webhook push with
-- entity_type = 'commit' (see lib/services/vcs.ts `ingest` and the
-- activityEntityValues enum in lib/db/schema.ts). Mirrors migration 0037,
-- which did the same thing for 'branch'.
--
...
-- 0018 + 'branch' added by 0037), changing only the entity_type CHECK to
-- include 'commit'.
...
CREATE TABLE "project_activity_new" (
  ...
  "entity_type" TEXT NOT NULL CHECK ("entity_type" IN ('project', 'request', 'task', 'note', 'branch', 'commit')),
